...
from cfp_crawl.classifier import classify_url, URLClass
...
class BaseCfpSpider(scrapy.spiders.CrawlSpider):
    ...
    def parse_conference_page(self, response):
        ...
        if 'application/pdf' in content_type:
            ...
            for link_elem in conference_home_links:
                ...
                url_class = classify_url(full_link)
    ...
    def process_wikiconf(self, response):
        ...
        if url:
            if parsed_conference['wayback_url']:
                ...
                return self.process_conference_url(url, row_id)
    ...
    def handle_request_error(self, err):
        ...
        if "wayback" not in err.request.url:
            return scrapy.spiders.Request(url=err.request.meta['wayback_url'], callback=self.parse_conference_page,
                                                errback=self.handle_request_error,
                                                dont_filter=True)