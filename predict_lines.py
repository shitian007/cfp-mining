...
class LinePredictor:
    ...
    def predict_lines(self, cur: 'Cursor', df, confidence_threshold):
...
def predict_conference_lines(db_filepath, svm_filepath, tfidf_filepath, conf_thresh):
    """ Predict pagelines for Conference and saves to database
    """
    cnx = sqlite3.connect(db_filepath)
    cur = cnx.cursor()
    conf_ids = cur.execute("SELECT id FROM WikicfpConferences WHERE accessible LIKE '%Accessible%' ORDER BY id").fetchall()[0:1]
    for conf_id in conf_ids:
        ...
        for confpage in confpages:
            ...
            line_predictor.predict_lines(cur, pagelines_df, conf_thresh)
...
predict_conference_lines(args.db_filepath, SVM_FILEPATH, TFIDF_FILEPATH, CONF_THRESH)
